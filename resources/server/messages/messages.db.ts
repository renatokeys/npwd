...
export class _MessagesDB {
  ...
  async getMessageGroups(userIdentifier: string): Promise<UnformattedMessageGroup[]> {
    ...
    SELECT
      ...
      ${config.database.playerTable}.${config.database.numberColumn},
    ...
    LEFT OUTER JOIN npwd_phone_contacts on REGEXP_REPLACE(npwd_phone_contacts.number, '[^0-9]', '') = REGEXP_REPLACE(${config.database.playerTable}.${config.database.numberColumn}, '[^0-9]', '') AND npwd_phone_contacts.identifier = ?
  ...
  async getIdentifierFromPhoneNumber(phoneNumber: string): Promise<string> {
    const query = `
      ...
      WHERE REGEXP_REPLACE(${config.database.numberColumn}, '[^0-9]', '') = ?
      LIMIT 1
    `;
