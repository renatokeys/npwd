...
export async function generatePhoneNumber(identifier: string): Promise<string> {
  const getQuery = `SELECT ${config.database.numberColumn} FROM ${config.database.playerTable} WHERE ${config.database.identifierColumn} = ?`;
  const [results] = await DbInterface._rawExec(getQuery, [identifier]);
  const result = <any[]>results;
  const phoneNumber = result[0]?.[config.database.numberColumn];
  ...
  if (!phoneNumber)
    if (!phoneNumber) {
      ...
      const query = `UPDATE ${config.database.playerTable} SET ${config.database.numberColumn} = ? WHERE ${config.database.identifierColumn} = ?`;
